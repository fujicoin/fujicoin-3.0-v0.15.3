...
const char * const FUJICOIN_PID_FILENAME = "fujicoind.pid";
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No fujicoin.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override fujicoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", FUJICOIN_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Fujicoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Fujicoin Core") == std::string::npos) {
