...
const char* BIP71_MIMETYPE_PAYMENT = "application/fujicoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/fujicoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/fujicoin-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the fujicoin: URI contains a payment request, we are not able to detect the
        ...
        if (arg.startsWith(FUJICOIN_IPC_PREFIX, Qt::CaseInsensitive)) // fujicoin: URI
        {
            ...
            if (GUIUtil::parseFujicoinURI(arg, &r) && !r.address.isEmpty())
            {
                CFujicoinAddress address(r.address.toStdString());
...
//
// OSX-specific way of handling fujicoin: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in fujicoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(FUJICOIN_IPC_PREFIX, Qt::CaseInsensitive)) // fujicoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            {
                CFujicoinAddress address(recipient.address.toStdString());
                if (!address.IsValid()) {
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CFujicoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom fujicoin addresses are not supported
        ...
        // Fujicoin amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(FujicoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
