// Copyright (c) 2011-2016 The Fujicoin Core developers
...
const QString FujicoinGUI::DEFAULT_WALLET = "~Default";

FujicoinGUI::FujicoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void FujicoinGUI::createActions()
{
    ...
    openAction->setStatusTip(tr("Open a fujicoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Fujicoin command-line options").arg(tr(PACKAGE_NAME)));
...
#ifdef ENABLE_WALLET
bool FujicoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FujicoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FujicoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FujicoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Fujicoin"); // default title
...
#ifdef ENABLE_WALLET
void FujicoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
...
void FujicoinGUI::setHDStatus(int hdEnabled)
{
...
void FujicoinGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    {
        max_width = qMax(max_width, fm.width(FujicoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (FujicoinUnits::Unit u : FujicoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FujicoinUnits::name(u)), this);
...
{
    setText(FujicoinUnits::name(newUnits));
}
