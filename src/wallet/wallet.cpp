...
{
    // for now we use a fixed keypath scheme of m/75'/0'/k
    CKey key;                      //master key seed (256bit)
    CExtKey masterKey;             //hd master key
    CExtKey accountKey;            //key at m/75'
    CExtKey chainChildKey;         //key at m/75'/0' (external) or m/75'/1' (internal)
    CExtKey childKey;              //key at m/75'/0'/<n>'
    ...
    // derive m/75'/0' (external chain) OR m/75'/1' (internal chain)
    ...
    do {
        ...
        if (internal) {
            ...
            metadata.hdKeypath = "m/75'/1'/" + std::to_string(hdChain.nInternalChainCounter) + "'";
            hdChain.nInternalChainCounter++;
        }
        else {
            chainChildKey.Derive(childKey, hdChain.nExternalChainCounter | BIP32_HARDENED_KEY_LIMIT);
            metadata.hdKeypath = "m/75'/0'/" + std::to_string(hdChain.nExternalChainCounter) + "'";
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CFujicoinAddress(CScriptID(redeemScript)).ToString();
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-fujicoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CFujicoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CFujicoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CFujicoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CFujicoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CFujicoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CFujicoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CFujicoinAddress(dest).ToString(), key);
}
