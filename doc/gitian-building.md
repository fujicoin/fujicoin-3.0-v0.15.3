...
*Setup instructions for a Gitian build of Fujicoin Core using a Debian VM or physical system.*
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-fujicoin-host/README.md).
...
in the fujicoin repository under 'Fetch and create inputs' to install sources which require
...
To build Fujicoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fujicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fujicoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/fujicoin/fujicoin
...
For example:
```bash
URL=https://github.com/laanwj/fujicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fujicoin=${COMMIT} --url fujicoin=${URL} ../fujicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fujicoin=${COMMIT} --url fujicoin=${URL} ../fujicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fujicoin=${COMMIT} --url fujicoin=${URL} ../fujicoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fujicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fujicoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fujicoin
SIGPATH=/some/root/path/fujicoin-detached-sigs

./bin/gbuild --url fujicoin=${BTCPATH},signature=${SIGPATH} ../fujicoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fujicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fujicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fujicoin-osx-build.assert
```
...
[fujicoin-core/gitian.sigs](https://github.com/fujicoin-core/gitian.sigs/) repository, or if that's not possible create a pull
