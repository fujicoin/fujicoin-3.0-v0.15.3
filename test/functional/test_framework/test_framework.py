...
class FujicoinTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing fujicoind/fujicoin-cli (default: %default)")
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'fujicoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "fujicoind should have exited with an error"
                else:
                    assert_msg = "fujicoind should have exited with expected error " + expected_msg
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run fujicoinds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("FUJICOIND", "fujicoind"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in fujicoin.conf
...
class ComparisonTestFramework(FujicoinTestFramework):
    """Test framework for doing p2p comparison testing

    Sets up some fujicoind binaries:
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("FUJICOIND", "fujicoind"),
                          help="fujicoind binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("FUJICOIND", "fujicoind"),
                          help="fujicoind binary to use for reference nodes (if any)")
