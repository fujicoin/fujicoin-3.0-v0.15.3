#!/bin/bash
# Copyright (c) 2013 The Fujicoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Fujicoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Fujicoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Fujicoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Fujicoin 0.7 datadir."
    ;;
  3)
    echo "Detected Fujicoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Fujicoin 0.8 datadir."
    ;;
esac
