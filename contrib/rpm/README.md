...
The RPM spec file provided here is for Fujicoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://fujicoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Fujicoin git repository](https://github.com/fujicoin/fujicoin) but are not part of
...
Currently only `Source10` is used, for the example `fujicoin.conf` file.
...
in the fujicoin git repository. At present only an SVG version of the Fujicoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the fujicoin
...
needed when 0.12.1 is released, a proper fix is already in the Fujicoin git
master branch.
...
idea to build Fujicoin Core against a static version of that library compiled
...
The `fujicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `fujicoin-server` package. At this point, most RPM based
...
The `fujicoin-server` package should have SELinux support. How to properly do
...
Fujicoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/fujicoin/). He has been
packaging Fujicoin for Fedora at least since 2012.
...
4. I renamed the `fujicoin` package that contains the Qt GUI to `fujicoin-core` as
that appears to be how the general population refers to it, in contrast to
`fujicoin-xt` or `fujicoin-classic`. I wanted to make sure the general population
